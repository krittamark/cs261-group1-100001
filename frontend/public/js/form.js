document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function loadApplicationData(applicationId) {
        try {
            const response = await fetch(`http://localhost:8080/api/requests/${applicationId}`, {
                method: "GET",
                headers: {
    ...
    document.querySelectorAll("form.main-form").forEach((form) => {
        form.querySelector(".draft-button").addEventListener("click", function (event) {
            event.preventDefault();
            const formStatus = "draft";
            if (validateForm(form)) {
                submitForm(form, "แบบร่างของคุณถูกบันทึกแล้ว", "แบบร่างของคุณถูกบันทึกสำเร็จ", formStatus);
            } else {
                showPopup("การบันทึกไม่สำเร็จ", "กรุณากรอกข้อมูลที่จำเป็นให้ครบถ้วน", false);
            }
        });

        form.querySelector(".submit-button").addEventListener("click", function (event) {
            event.preventDefault();
            const formStatus = "pending";
            if (validateForm(form)) {
                submitForm(form, "การยื่นคำร้องสำเร็จ", "คำร้องของคุณกำลังรอการอนุมัติ", formStatus);
            } else {
                showPopup("การยื่นคำร้องไม่สำเร็จ", "กรุณากรอกข้อมูลที่จำเป็นให้ครบถ้วน", false);
            }
        });
        ...
        function submitForm(form, title, message, formStatus) {
            ...
            fetch("http://localhost:8080/api/requests", {
                ...
                .catch((error) => showPopup("การยื่นคำร้องไม่สำเร็จ", "เกิดข้อผิดพลาดในการส่งคำร้อง", false));
        }
    });
});
